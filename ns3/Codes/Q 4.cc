...
int main ()
{
	...
	NetDeviceContainer dv1;
  	dv1 = p2p.Install (ringNodes.Get(0),ringNodes.Get(1));
	
	NetDeviceContainer dv2;
  	dv2 = p2p.Install (ringNodes.Get(1),ringNodes.Get(2));

	NetDeviceContainer dv3;
  	dv3 = p2p.Install (ringNodes.Get(2),ringNodes.Get(3));
	
	NetDeviceContainer dv4;
  	dv4 = p2p.Install (ringNodes.Get(3),ringNodes.Get(0));
	...
	address.SetBase ("10.1.4.0", "255.255.255.0");
  	...
  	ApplicationContainer clientApps = echoClient.Install (ringNodes.Get (2));
  	clientApps.Start (Seconds (2.0));
  	clientApps.Stop (Seconds (10.0));

  	Ipv4GlobalRoutingHelper::PopulateRoutingTables ();

  	AnimationInterface anim("abc4.xml");
